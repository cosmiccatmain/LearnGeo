...
(function (global) {
  ...
  var cfg = { scope: 'un', regions: [], length: 10 };
  var Q = null;      /* the run */
  ...
  function renderSetup() {
    ...
    head().innerHTML = '<div class="sidebar__title">Class quiz</div>' +
      '<div class="t-sm t-muted" style="margin-top:2px">Shade, click, write. Like the real thing.</div>';

    var pool = global.Quiz.pool({ scope: cfg.scope, regions: cfg.regions });
    side().innerHTML =
      '<div class="deck-summary">' +
        '<div class="deck-summary__n mono">' + pool.length + '</div>' +
        '<div class="deck-summary__l">countries in this pool</div>' +
      '</div>' +
      '<div class="divider"></div>' +
      '<span class="eyebrow">How a question runs</span>' +
      '<ol class="quiz-steps">' +
      ...
      '<div class="setup-panel">' +
        '<div class="t-center" style="margin-bottom:26px">' +
          ...
          '<p class="t-muted" style="margin-top:8px;font-size:14.5px">' +
            'You will be shown a shaded country and asked to write its name and capital.</p>' +
        '</div>' +
  ...
  function begin() {
    ...
    var pool = global.Quiz.pool({ scope: cfg.scope, regions: cfg.regions });
    if (pool.length < 3) { W.toast('Too few countries', 'Widen the regions or scope', I.info); return; }

    Q = {
      phase: 'running', i: 0,
      items: W.sample(pool, Math.min(cfg.length, pool.length)).map(function (c) {
        return { country: c, clicked: null, nameGiven: '', capGiven: '', nameOk: null, capOk: null, misclicks: 0 };
      }),
  ...
  function renderRound() {
    showPane('map');
    stage = item().clicked ? (item().nameOk === null ? 'name' : 'capital') : 'locate';
    paintMap();
    renderSidebar();
  }
  ...
  function paintMap() {
    ...
    /* Neighbours give the shaded country context to be clicked among. */
    var pool = global.Quiz.pool({ scope: cfg.scope, regions: cfg.regions });
    var near = pool.filter(function (c) {
      return c.name !== target.name && c.region === target.region;
    });
    var context = W.sample(near, Math.min(14, near.length));
    ...
    setHint(stage === 'locate'
      ...
      : (stage === 'marked' ? '' : 'Now write it in the sidebar'));
  }
  ...
  function onWrongClick(country) {
    ...
    item().misclicks += 1;
    W.Sound.wrong();
    W.toast('Not that one', 'The shaded country is the one to click', I.info, 1800);
  }
  ...
  function renderSidebar() {
    ...
    var html = '<ol class="quiz-track">' +
      step(1, 'Locate it on the map', it.clicked ? 'done' : (stage === 'locate' ? 'active' : 'todo')) +
    ...
    if (stage === 'locate') {
      html += '<div class="quiz-prompt">' +
        ...
        '<p>One country on the map is shaded gold. Click it to claim the question.</p>' +
    ...
    function mark(okFlag, answer, given) {
      return '<div class="feedback feedback--' + (okFlag ? 'right' : 'wrong') + '" style="margin-top:10px">' +
        ...
        (okFlag ? '' : '<p>You wrote “' + W.escapeHtml(given || '—') + '”. It is <b>' +
  ...
  function markedPanel(it) {
    ...
    return '<div class="qcard">' +
      '<div class="feedback feedback--' + (marks === 2 ? 'right' : marks === 1 ? 'wrong' : 'wrong') + '">' +
        ...
        '<p><b>' + W.escapeHtml(c.name) + '</b> — capital <b>' + W.escapeHtml(c.capital) + '</b>.</p>' +
  ...
  function pay(correct, anchor) {
    ...
    if (gains.level) {
      ...
      W.toast('Level ' + gains.level, 'Class quiz counts too', I.bolt);
    }
  ...
  function renderReport() {
    ...
    document.getElementById('quiz-report').innerHTML =
      '<div class="report">' +
        '<div class="score-hero">' +
          '<span class="eyebrow">Class quiz — marked</span>' +
        ...
        '<div class="breakdown">' +
          '<span class="eyebrow">Your paper</span>' +
        ...
        '<div class="row" style="gap:10px;margin-top:30px;justify-content:center">' +
          ...
          '<button class="btn btn--ghost btn--lg" id="qz-home">Back to study centre</button>' +
        '</div>' +
      '</div>';
    ...
    head().innerHTML = '<div class="sidebar__title">Marked</div>' +
      '<div class="t-sm t-muted" style="margin-top:2px">' + Q.marks + ' of ' + Q.outOf + ' marks</div>';
    side().innerHTML = '<span class="eyebrow">Revisit</span>' +
      (Q.items.filter(function (it) { return !it.nameOk || !it.capOk; }).length
        ...
        : '<div class="t-sm t-muted" style="margin-top:8px">Full marks. Nothing to revisit.</div>');
  ...
  function verdict(p) {
    ...
    if (p >= 90) return 'Top of the class';
    if (p >= 75) return 'Solid pass';
    if (p >= 60) return 'Scraped through';
    if (p >= 40) return 'Needs revision';
    return 'Back to the atlas';
  }

  function reset() { Q = null; stage = 'idle'; renderSetup(); }

  global.QuizMode = { start: start, reset: reset };
})(window);
