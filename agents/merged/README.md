...
Email and password work everywhere. On a device that supports passkeys you can also set one
up and sign in with your face, fingerprint or screen lock instead, with no password to
remember. Supabase Auth runs that, and `assets/js/passkey.js` adds the button. That file also
puts an eye on every password box so you can check what you typed. It watches for the dialogs
to appear rather than being called by `ui.js`, so the two don't get in each other's way.

### The admin panel

Adding `?admin` to the URL opens a number pad. The four-digit codes live in Supabase and are
checked there (`assets/js/admin-pin.js`), so the page never holds one, and if it can't reach
the server it refuses rather than falling back to anything. Unlocking it lets you hand out the
two things the app won't award on its own: the verified seal and diamonds. It only changes
this browser's save. Four digits is a speed bump to keep students out, not real security.
...
assets/css/auth.css             sign in, create account and password dialogs
assets/css/brand.css            the LearnGeo mark
...
assets/js/relief.js             the ridgeline relief behind the Geo Classroom headline
...
assets/js/passkey.js            passkey sign-in and the eye on password boxes
assets/js/portal.js             study center home screen, built around the map
assets/js/admin.js              ?admin panel: grants and removes the verified seal and diamonds
assets/js/admin-pin.js          checks the admin code against Supabase
