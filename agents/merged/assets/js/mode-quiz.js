...
(function (global) {
  ...
  function onWrongClick(country) {
    ...
    renderSidebar();   /* the running "N wrong clicks" line lives there */
  }
  ...
  function submit() {
    ...
    if (stage === 'name') {
      ...
      pay(it.capOk, cap, it.country.name);
      stage = 'marked';
      paintMap();
      renderSidebar();
    }
  ...
  /* Each question is paid twice, once for the country and once for the
     capital. award() steps the Leitner box every time it is handed a country
     name, so only the second half names it: otherwise one question would move
     mastery two boxes. XP, gems and the streak still count both halves.
     `quiet` silences the second sound when a pair is scored together, so a
     skip marks two wrong answers without playing the same sound over itself. */
  function pay(correct, anchor, country, quiet) {
    var gains = W.award(correct, { country: country || null, baseXp: 12, baseGems: 5 });
