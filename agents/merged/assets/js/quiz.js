...
(function (global) {
  ...
  function generate(config) {
    /* Two pools, and they are not the same one. The questions are asked about
       exactly what was requested; the wrong answers may have to come from
       further afield, because a hand-picked list of three cannot supply three
       distractors on its own. Drawing the questions from the widened pool too
       is what used to turn "only my list" into a random world test whenever
       the list was shorter than five. */
    var wanted = pool({ regions: config.regions, scope: config.scope,
                        countries: config.countries, weakFirst: !!config.weakFirst });

    /* Nothing matched at all: a quiz on the whole scope beats an empty one. */
    if (!wanted.length) wanted = pool({ scope: config.scope });

    /* Only the distractors borrow the wider field. It never decides what
       gets asked. */
