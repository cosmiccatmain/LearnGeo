...
(function (global) {
  ...
  function start() {
    /* the shape file can still be in flight, which draws a circle instead of
       an outline — so redraw the revealed card once it lands */
    global.GeoMap.loadShapes().then(function () {
      if (flipped && document.body.classList.contains('view-cards')) paintMap(true);
    });
    if (!built) build();
    renderSidebar();
    renderCard();
  ...
  document.addEventListener('keydown', function (e) {
    ...
    /* Space and Enter belong to whatever button has focus, so Deck settings
       and Rebuild deck still work from the keyboard. The number keys are ours
       either way, and the card itself is a div, so it still flips on Space. */
