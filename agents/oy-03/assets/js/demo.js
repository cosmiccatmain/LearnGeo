...
(function (global) {
  ...
  function draw() {
    ...
    root.innerHTML =
      ...
      '</div>' +
      '<div class="demo__foot" aria-live="polite"><span class="demo__note">Pick one.</span></div>';
  ...
  function onKey(e) {
    ...
    var card = root && root.closest ? root.closest('.demo') : null;
    if (!card) return;
    ...
    if (i === -1) return;
    var opt = root.querySelectorAll('.option')[i];
