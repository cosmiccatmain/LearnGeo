...
(function (global) {
  ...
  /* Your score for an assignment is the last attempt, because that is the
     one that was handed in and the one your teacher's gradebook shows.
     Older saves that only kept a best score still have one. */
  function score(a) {
    if (a && a.last && typeof a.last.pct === 'number') return a.last.pct;
    return (a && a.best) || 0;
  }
  ...
  function render(fromSync) {
    ...
    host.innerHTML =
      '<div class="cr">' +
        ...
        '<div id="cl-body">' + pendingNotice() +
          (tab === 'grades' ? gradesPanel() : classworkPanel()) + '</div>' +
      '</div>';
  ...
  /* A join link for a class you are not in. Without this the code arrives,
     the old class is drawn as usual and nothing says the link did anything. */
  function pendingNotice() {
    if (!pending || pending === classCode()) return '';
    return '<div class="signin-hint" style="margin:0 0 14px">' + I.info +
      '<span>That link was for class <b class="mono">' + W.escapeHtml(pending) + '</b>. ' +
      'You are in ' + W.escapeHtml(className() || 'another class') + ' at the moment.</span>' +
      '<button class="btn btn--accent btn--sm" id="cl-switch">Switch class</button></div>';
  }

  function switchClass() {
    var code = pending, e = enrolled();
    if (!code || !e) return;
    global.UI.modal({
      title: 'Switch to ' + code, icon: I.users,
      body: '<p class="t-muted">You will leave <b>' + W.escapeHtml(e.className || 'your class') +
        '</b> and join the one this link points at. Scores you have already sent stay with ' +
        'your old teacher, and anything you finished stays on your list.</p>',
      actions: [
        { label: 'Stay here', cls: 'btn--ghost', close: true },
        { label: 'Switch', cls: 'btn--primary', close: true, onClick: function () { doSwitch(code, e); } }
      ]
    });
  }

  function doSwitch(code, e) {
    var C = global.Cloud;
    var name = e.name || W.state.profile.displayName;
    if (!C || !C.ready) {
      W.toast('Not connected', 'Joining a class needs a connection. Try again in a moment.', I.info, 4200);
      return;
    }
    /* join first: if the new code turns out not to work, the class they are
    ...
    C.joinClass(code, name).then(function (klass) {
      ...
      W.toast('Couldn’t switch', C.friendly(err), I.info, 4200);
    });
  }
  ...
  function join(host) {
    ...
    /* signed in, but the account and this device are still being compared.
       The sign-in dialog would only say you are already signed in and drop
       the join on the floor, so wait for it instead. */
    if (!C.ready && C.signedIn) {
      W.state.profile.displayName = name;
      W.saveNow();
      pending = code;
      return joinError(host, 'Not connected yet',
        'Your account is still catching up. Give it a moment and press Join again.');
    }
  ...
  function wire(host) {
    ...
    /* the id, not the row's place in the list: a sync between this page
       being drawn and the button being pressed moves the rows around */
    W.$$('[data-start]', host).forEach(function (b) {
      b.addEventListener('click', function () {
        var a = byId(b.dataset.start);
        if (a) A.run(a); else gone();
      });
    });
    W.$$('[data-send]', host).forEach(function (b) {
      b.addEventListener('click', function () {
        var a = byId(b.dataset.send);
        if (a) A.send(a); else gone();
      });
    });
  ...
  /* It was on the list when this page drew and it is not on it now. */
  function gone() {
    W.toast('That is not on your list any more',
            'Your teacher changed the classwork while this page was open', I.info, 4200);
    render();
  }
      ...
      (box.length
        ? box.map(function (a) {
            ...
            return '<div class="cw-row">' +
              ...
              (a.done
                ? '<div class="cw-row__meta"><b>' + score(a) + '%</b>' +
                    (a.attempts > 1 ? a.attempts + ' tries' : 'your score') + '</div>'
                : '<div class="cw-row__meta"><b>–</b>not started</div>') +
              '<div class="row" style="gap:6px">' +
                (a.done && a.last
                  ? '<button class="btn btn--ghost btn--sm" data-send="' + id + '">' + I.key +
                      (a.last.sent ? ' Sent' : ' Send') + '</button>'
                  : '') +
                '<button class="btn btn--' + (a.done ? 'ghost' : 'primary') + ' btn--sm" data-start="' + id + '">' +
                  (a.done ? 'Retry' : 'Start') + '</button>' +
                '<button class="icon-btn" data-drop="' + id + '" title="Remove">' + I.close + '</button>' +
              '</div></div>';
          }).join('')
  ...
  function gradesPanel() {
    ...
    var mean = avg(done.map(score));
    var best = done.slice().sort(function (a, b) { return score(b) - score(a); })[0];
    var worst = done.slice().sort(function (a, b) { return score(a) - score(b); })[0];
    ...
    return '<h3 style="font-size:18px;margin-bottom:4px">Grades</h3>' +
      ...
      '<div class="an-grid" style="grid-template-columns:repeat(3,1fr);margin-bottom:20px">' +
        ...
        stat(score(best) + '%', 'best', best.title) +
        stat(score(worst) + '%', 'lowest', worst.title) +
      '</div>' +
          ...
          return '<div class="an-bar">' +
            ...
            '<div class="an-bar__n">' +
              (a.last && a.last.correct !== null
                ? a.last.correct + '/' + a.last.total + ' · ' : '') + p + '%</div>' +
            '<div class="an-bar__track" style="grid-column:1/-1">' +
              '<div class="an-bar__fill" style="width:' + p + '%;background:' +
                (p >= 80 ? 'var(--success)' : p >= 50 ? 'var(--gold)' : 'var(--danger)') +
              '"></div></div></div>';
        }).join('') +
      '</div>' +
  ...
  function checkForWork() {
    ...
    C.studentSync().then(function () {
      /* counting the list before and after hides one piece of work arriving
         while another is taken back, so the merge says what it did */
      var m = C.lastInboxChange || { added: 0, removed: 0 };
      var from = className() || 'your teacher';
      render(true);
      if (!m.added && !m.removed) return W.toast('You are up to date', '', I.info);
      var title = m.added
        ? m.added + ' new assignment' + (m.added === 1 ? '' : 's')
        : m.removed + ' assignment' + (m.removed === 1 ? '' : 's') + ' taken back';
      var body = m.added
        ? (m.removed ? 'From ' + from + '. ' + m.removed +
                       (m.removed === 1 ? ' other was' : ' others were') + ' taken back'
                     : 'Just arrived from ' + from)
        : from + ' took ' + (m.removed === 1 ? 'it' : 'them') + ' off your list';
      W.toast(title, body, m.added ? I.check : I.info);
    }, function (e) {
